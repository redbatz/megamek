...
import org.redbat.roguetech.megamek.client.TimerSingleton;
import org.redbat.roguetech.megamek.client.bot.princess.BotGeometry;
import org.redbat.roguetech.megamek.client.bot.princess.PathEnumerator;
import org.redbat.roguetech.megamek.client.bot.princess.Princess;
import org.redbat.roguetech.megamek.client.event.BoardViewEvent;
import org.redbat.roguetech.megamek.client.event.BoardViewListener;
import org.redbat.roguetech.megamek.client.event.MechDisplayEvent;
import org.redbat.roguetech.megamek.client.ui.IBoardView;
import org.redbat.roguetech.megamek.client.ui.IDisplayable;
import org.redbat.roguetech.megamek.client.ui.Messages;
import org.redbat.roguetech.megamek.client.ui.SharedUtility;
import org.redbat.roguetech.megamek.client.ui.swing.*;
import org.redbat.roguetech.megamek.client.ui.swing.util.*;
import org.redbat.roguetech.megamek.client.ui.swing.widget.MegamekBorder;
...
import org.redbat.roguetech.megamek.common.actions.*;
...
import org.redbat.roguetech.megamek.common.pathfinder.BoardClusterTracker;
import org.redbat.roguetech.megamek.common.pathfinder.BoardClusterTracker.BoardCluster;
import org.redbat.roguetech.megamek.common.preference.IClientPreferences;
import org.redbat.roguetech.megamek.common.preference.IPreferenceChangeListener;
import org.redbat.roguetech.megamek.common.preference.PreferenceChangeEvent;
import org.redbat.roguetech.megamek.common.preference.PreferenceManager;
import org.redbat.roguetech.megamek.common.util.FiringSolution;
...
import static org.redbat.roguetech.megamek.common.IGame.Phase.*;
    ...
    */
    ...
    private boolean shouldIgnoreKeyCommands() {
        return getChatterBoxActive() || !isVisible()
               || (game.getPhase() == PHASE_LOUNGE)
               || (game.getPhase() == PHASE_END_REPORT)
               || (game.getPhase() == PHASE_MOVEMENT_REPORT)
               || (game.getPhase() == PHASE_TARGETING_REPORT)
               || (game.getPhase() == PHASE_FIRING_REPORT)
               || (game.getPhase() == PHASE_PHYSICAL_REPORT)
               || (game.getPhase() == PHASE_OFFBOARD_REPORT)
               || (game.getPhase() == PHASE_INITIATIVE_REPORT)
               || shouldIgnoreKeys;
    }
    ...
    public synchronized void paintComponent(Graphics g) {
        ...
        if ((game.getPhase() == PHASE_MOVEMENT) && !useIsometric()) {
        ...
        if ((game.getPhase() == PHASE_SET_ARTYAUTOHITHEXES)
                && (showAllDeployment)) {
            drawAllDeployment(g);
        }
        ...
        if ((game.getPhase() == PHASE_FIRING) ||
            (game.getPhase() == PHASE_OFFBOARD)) {
    ...
    private void renderMovementBoundingBox(Graphics2D g) {
        if(selectedEntity != null) {
            ...
            BotGeometry.ConvexBoardArea cba = pathEnum.getUnitMovableAreas().get(this.selectedEntity.getId());
    ...
    private void updateShadowMap() {
        ...
        if (game.getPhase() == PHASE_UNKNOWN) return;
        ...
        for (Enumeration<MoveStep> i = md.getSteps(); i.hasMoreElements(); ) {
            ...
            if ((null != previousStep)
                && ((step.getType() == MovePath.MoveStepType.UP)
                    ...
                    || (step.getType() == MovePath.MoveStepType.ACC)
                    || (step.getType() == MovePath.MoveStepType.DEC)
                    || (step.getType() == MovePath.MoveStepType.ACCN)
            ...
            if (previousStep != null
                    ...
                    && (game.useVectorMove()
                            ...
                            || (step.getType() == MovePath.MoveStepType.CONVERT_MODE
                            && previousStep.getType() == MovePath.MoveStepType.CONVERT_MODE)
                            || step.getType() == MovePath.MoveStepType.BOOTLEGGER)) {
                pathSprites.get(pathSprites.size() - 1).setHidden(true);
            }
    ...
    private GameListener gameListener = new GameListenerAdapter() {
        ...
        public void gameEntityNew(GameEntityNewEvent e) {
            updateEcmList();
            redrawAllEntities();
            if (game.getPhase() == PHASE_MOVEMENT) {
                refreshMoveVectors();
            }
        }
    ...
    public String getToolTipText(MouseEvent e) {
            ...
            if (mhex.containsTerrain(Terrains.BUILDING)) {
                ...
                if (clientgui == null) {
                    ...
                    txt.append(Messages.getString("BoardView1.Tooltip.Building", new Object[] { //$NON-NLS-1$
                            ...
                            Building.BasementType.getType(mhex.terrainLevel(Terrains.BLDG_BASEMENT_TYPE)).toString()
                    }));
                } else {
                    Building bldg = game.getBoard().getBuildingAt(mcoords);
        ...
        if ((game.getPhase() == PHASE_SET_ARTYAUTOHITHEXES) && (mhex != null)) {
        ...
        final IGame.Phase currPhase = game.getPhase();
