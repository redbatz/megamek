...
public class FieldofFireSprite extends MovementEnvelopeSprite {
    ...
    public void prepare() {
        ...
        case 1: // 2 adjacent borders
            drawBorderXC(graph, HexDrawUtilities.getHexCrossArea01(bDir[borders], borderW),
                    HexDrawUtilities.getHexCrossLine01(bDir[borders], borderW));
            break;
        case 2: // 3 adjacent borders
            drawBorderXC(graph, HexDrawUtilities.getHexCrossArea012(bDir[borders], borderW),
                    HexDrawUtilities.getHexCrossLine012(bDir[borders], borderW));
            break;
        case 3: // 4 adjacent borders
            drawBorderXC(graph, HexDrawUtilities.getHexCrossArea0123(bDir[borders], borderW),
                    HexDrawUtilities.getHexCrossLine0123(bDir[borders], borderW));
            break;
        case 4: // twice two adjacent borders
            drawBorderXC(graph, HexDrawUtilities.getHexCrossArea01(bDir[borders], borderW),
                    HexDrawUtilities.getHexCrossLine01(bDir[borders], borderW));
            drawBorderXC(graph, HexDrawUtilities.getHexCrossArea01(bDir[borders]+3, borderW),
                    HexDrawUtilities.getHexCrossLine01(bDir[borders]+3, borderW));
            break;
        ...
        case 7: // two adjacent borders and one lone (other hexface)
            ...
            break;
        case 8: 
            drawBorderXC(graph, HexDrawUtilities.getHexCrossArea01234(bDir[borders], borderW),
                    HexDrawUtilities.getHexCrossLine01234(bDir[borders], borderW));
            break;
        default:
    ...
    private void drawLoneBorder(Graphics2D graph, int dir) {
        // 1) thick transparent border
        graph.setColor(fillColor);
        graph.fill(HexDrawUtilities.getHexBorderArea(dir, HexDrawUtilities.CUT_BORDER, borderW));

        // 2) thin dashed line border
        graph.setColor(lineColor);
        graph.draw(HexDrawUtilities.getHexBorderLine(dir));
    }
        ...
        for (int i = 0; i < 6; i++) {
            if ((borders & (1 << i)) != 0) {
                ...
                int cut = ((borders & (1 << ((i + 1) % 6))) == 0)? 
                    HexDrawUtilities.CUT_RBORDER : HexDrawUtilities.CUT_RINSIDE;
                cut |= ((borders & (1 << ((i + 5) % 6))) == 0)? 
                    HexDrawUtilities.CUT_LBORDER : HexDrawUtilities.CUT_LINSIDE;

                graph.setColor(fillColor);
                graph.fill(HexDrawUtilities.getHexBorderArea(i, cut, borderW));
                
                // 2) thin dashed line border
                graph.setColor(lineColor);
                graph.draw(HexDrawUtilities.getHexBorderLine(i, cut, lineThickness/2));
            }
        } 
    }
