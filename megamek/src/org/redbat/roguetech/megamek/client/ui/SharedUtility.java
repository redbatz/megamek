...
public class SharedUtility {
    ...
    private static Object getAeroSpecificPSRList(MovePath md, boolean stringResult) {
        ...
        for (final Enumeration<MoveStep> i = md.getSteps(); i.hasMoreElements();) {
            ...
            if (step.getType() == MovePath.MoveStepType.VTAKEOFF) {
                rollTarget = ((IAero)entity).checkVerticalTakeOff();
                checkNag(rollTarget, nagReport, psrList);
            }

            //check for landing
            if (step.getType() == MovePath.MoveStepType.LAND) {
                rollTarget = ((IAero) entity).checkLanding(moveType,
                        step.getVelocity(), curPos, curFacing, false);
                checkNag(rollTarget, nagReport, psrList);
            }
            
            if (step.getType() == MovePath.MoveStepType.VLAND) {
                rollTarget = ((IAero) entity).checkLanding(moveType,
        ...
        // Atmospheric checks
        if (!game.getBoard().inSpace() && !md.contains(MovePath.MoveStepType.LAND)
                && !md.contains(MovePath.MoveStepType.VLAND)) {
    ...
    public static MovePath moveAero(MovePath md, Client client) {
        ...
        if ((md.getLastStep() != null)
                && (md.getLastStep().getType() == MovePath.MoveStepType.RAM)) {
            isRamming = true;
        }
        ...
        if (game.useVectorMove()) {
            ...
            while (vel > 0) {
                ...
                if (!game.getBoard().contains(md.getFinalCoords())) {
                    ...
                    if(game.getOptions().booleanOption(OptionsConstants.ADVAERORULES_RETURN_FLYOVER)) {
                        ...
                        if (entity instanceof TeleMissile) {
                            md.addStep(MovePath.MoveStepType.OFF);
                        } else {
                            md.addStep(MovePath.MoveStepType.RETURN);
                        }
                    } else {
                        md.addStep(MovePath.MoveStepType.OFF);
                    }
                ...
                if (a.isRandomMove()) {
                    ...
                    switch (roll) {
                    case 1:
                        md.addStep(MovePath.MoveStepType.TURN_LEFT);
                        md.addStep(MovePath.MoveStepType.TURN_LEFT);
                        break;
                    case 2:
                        md.addStep(MovePath.MoveStepType.TURN_LEFT);
                        break;
                    case 5:
                        md.addStep(MovePath.MoveStepType.TURN_RIGHT);
                        break;
                    case 6:
                        md.addStep(MovePath.MoveStepType.TURN_RIGHT);
                        md.addStep(MovePath.MoveStepType.TURN_RIGHT);
            ...
            if (oldmd.contains(MovePath.MoveStepType.LAUNCH)) {
            ...
            if (oldmd.contains(MovePath.MoveStepType.UNDOCK)) {
                // since launches have to be the last step
                MoveStep lastStep = oldmd.getLastStep();
                if (lastStep.getType() == MovePath.MoveStepType.UNDOCK) {
                    md.addStep(lastStep.getType(), lastStep.getLaunched());
                }
            }
        }
    ...
    private static MovePath addSteps(MovePath md, Client client) {
        ...
        if ((lastStep != null)
                && ((lastStep.getType() == MovePath.MoveStepType.LAUNCH) || (lastStep
                        .getType() == MovePath.MoveStepType.RECOVER) || (lastStep
                        .getType() == MovePath.MoveStepType.UNDOCK))) {
            md.removeLastStep();
        }
        ...
        for (int i = 1; i < in.size(); i++) {
            ...
            if (diff == 0) {
                ...
                md.addStep(MovePath.MoveStepType.LATERAL_RIGHT);
            } else if ((diff == -2) || (diff == 4)) {
                md.addStep(MovePath.MoveStepType.LATERAL_RIGHT_BACKWARDS);
            } else if ((diff == -1) || (diff == 5)) {
                md.addStep(MovePath.MoveStepType.LATERAL_LEFT);
            } else if ((diff == 2) || (diff == -4)) {
                md.addStep(MovePath.MoveStepType.LATERAL_LEFT_BACKWARDS);
            } else if ((diff == 3) || (diff == -3)) {
                md.addStep(MovePath.MoveStepType.BACKWARDS);
            }
            current = c;
        ...
        if (!leftMap && (lastStep != null) && (lastStep.getType() == MovePath.MoveStepType.LAUNCH)) {
            md.addStep(MovePath.MoveStepType.LAUNCH, lastStep.getLaunched());
        }
        
        if (!leftMap && (lastStep != null) && (lastStep.getType() == MovePath.MoveStepType.UNDOCK)) {
            md.addStep(MovePath.MoveStepType.UNDOCK, lastStep.getLaunched());
        }

        if (!leftMap && (lastStep != null) && (lastStep.getType() == MovePath.MoveStepType.RECOVER)) {
            md.addStep(MovePath.MoveStepType.RECOVER, lastStep.getRecoveryUnit(), -1);
        }
