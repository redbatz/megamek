...
import org.redbat.roguetech.megamek.client.event.BoardViewEvent;
import org.redbat.roguetech.megamek.client.ui.Messages;
import org.redbat.roguetech.megamek.client.ui.SharedUtility;
import org.redbat.roguetech.megamek.client.ui.swing.util.CommandAction;
import org.redbat.roguetech.megamek.client.ui.swing.util.KeyCommandBind;
import org.redbat.roguetech.megamek.client.ui.swing.util.MegaMekController;
import org.redbat.roguetech.megamek.client.ui.swing.util.TurnTimer;
...
import org.redbat.roguetech.megamek.common.actions.AirmechRamAttackAction;
import org.redbat.roguetech.megamek.common.actions.ChargeAttackAction;
import org.redbat.roguetech.megamek.common.actions.DfaAttackAction;
import org.redbat.roguetech.megamek.common.actions.RamAttackAction;
import org.redbat.roguetech.megamek.common.event.GamePhaseChangeEvent;
import org.redbat.roguetech.megamek.common.event.GameTurnChangeEvent;
import org.redbat.roguetech.megamek.common.options.GameOptions;
import org.redbat.roguetech.megamek.common.options.IOptions;
import org.redbat.roguetech.megamek.common.options.OptionsConstants;
import org.redbat.roguetech.megamek.common.pathfinder.AbstractPathFinder;
import org.redbat.roguetech.megamek.common.pathfinder.LongestPathFinder;
import org.redbat.roguetech.megamek.common.pathfinder.ShortestPathFinder;
...
public class MovementDisplay extends StatusBarPhaseDisplay {
                            ...
                            if (gear != MovementDisplay.GEAR_JUMP) {
                                if (!((cmd.getLastStep() != null)
                                        ...
                                        && (cmd.getLastStep().getType() 
                                                == MovePath.MoveStepType.LAY_MINE))) {
                                    clear();
                                }
                                if (!cmd.isJumping()) {
                                    cmd.addStep(MovePath.MoveStepType.START_JUMP);
                                }
    ...
    private void updateAeroButtons() {
        if (ce() != null && ce().isAero()) {
            ...
            setEvadeAeroEnabled(cmd != null && !cmd.contains(MovePath.MoveStepType.EVADE));
    ...
    private void removeLastStep() {
        ...
        if (cmd.length() == 0) {
            ...
            if ((gear == MovementDisplay.GEAR_JUMP) && 
                ...
                cmd.addStep(MovePath.MoveStepType.CONVERT_MODE);
            }
        } else {
            clientgui.bv.drawMovementData(ce(), cmd);
            clientgui.bv.setWeaponFieldofFire(ce(), cmd);
    ...
    public synchronized void ready() {
        ...
        if (cmd.contains(MovePath.MoveStepType.VTAKEOFF)
                || cmd.contains(MovePath.MoveStepType.TAKEOFF)) {
        ...
        if (Compute.useSpheroidAtmosphere(clientgui.getClient().getGame(), ce()) 
                && !cmd.contains(MovePath.MoveStepType.HOVER)
                && !cmd.contains(MovePath.MoveStepType.VLAND)
                && !cmd.contains(MovePath.MoveStepType.UP)
                && !cmd.contains(MovePath.MoveStepType.DOWN)) {
            ConfirmDialog nag = new ConfirmDialog(clientgui.frame,
                    Messages.getString("MovementDisplay.areYouSure"), //$NON-NLS-1$
        ...
        if (ce().isAirborne() || ce().isSpaceborne()) {
            if (!clientgui.getClient().getGame().useVectorMove()) {
                if (ce().isAero() && !((IAero)ce()).isOutControlTotal()) {
                    ...
                    if ((null != cmd)
                            && (cmd.contains(MovePath.MoveStepType.OFF) || cmd
                                    .contains(MovePath.MoveStepType.RETURN))) {
                        flyoff = true;
                    }
                    boolean landing = false;
                    if ((null != cmd) && cmd.contains(MovePath.MoveStepType.LAND)) {
                        landing = true;
                    }
                    boolean ejecting = false;
                    if ((null != cmd) && cmd.contains(MovePath.MoveStepType.EJECT)) {
                        ejecting = true;
                    }
        ...
        if ((ce() instanceof Infantry) && ((Infantry)ce()).hasMicrolite()
        		...
        		&& !cmd.contains(MovePath.MoveStepType.FORWARDS)
    ...
    private void currentMove(Coords dest) {
        if (shiftheld || (gear == GEAR_TURN)) {
            ...
            switch (dir) {
                case 0:
                    cmd.findSimplePathTo(dest, MovePath.MoveStepType.FORWARDS,
                            src.direction(dest), ce().getFacing());
                    break;
                case 1:
                    cmd.findSimplePathTo(dest, MovePath.MoveStepType.LATERAL_RIGHT,
                            src.direction(dest), ce().getFacing());
                    break;
                case 2:
                    ...
                    cmd.findSimplePathTo(dest,
                            MovePath.MoveStepType.LATERAL_LEFT_BACKWARDS,
                            src.direction(dest), ce().getFacing());
                    break;
                case 3:
                    cmd.findSimplePathTo(dest, MovePath.MoveStepType.BACKWARDS,
                            src.direction(dest), ce().getFacing());
                    break;
                case 4:
                    // TODO: backwards lateral shifts are switched:
                    ...
                    break;
                case 5:
                    cmd.findSimplePathTo(dest, MovePath.MoveStepType.LATERAL_LEFT,
                            src.direction(dest), ce().getFacing());
                    break;
            }
            ...
            cmd.findPathTo(dest, MovePath.MoveStepType.FORWARDS);
            ...
            if (start > 0 && !cmd.getStep(start - 1).isStrafingStep()) {
                while (start < cmd.length()
                        && cmd.getStep(start).getType() != MovePath.MoveStepType.FORWARDS) {
                    start++;
                }
            }
            ...
            cmd.findPathTo(dest, MovePath.MoveStepType.BACKWARDS);
        } else if (gear == GEAR_CHARGE) {
            cmd.findPathTo(dest, MovePath.MoveStepType.CHARGE);
            // The path planner shouldn't actually add the charge step
            if (cmd.getFinalCoords().equals(dest)
                && (cmd.getLastStep().getType() != MovePath.MoveStepType.CHARGE)) {
                cmd.removeLastStep();
                cmd.addStep(MovePath.MoveStepType.CHARGE);
            }
        } else if (gear == GEAR_DFA) {
            cmd.findPathTo(dest, MovePath.MoveStepType.DFA);
            // The path planner shouldn't actually add the DFA step
            if (cmd.getFinalCoords().equals(dest)
                && (cmd.getLastStep().getType() != MovePath.MoveStepType.DFA)) {
                cmd.removeLastStep();
                cmd.addStep(MovePath.MoveStepType.DFA);
            }
        } else if (gear == GEAR_SWIM) {
            cmd.findPathTo(dest, MovePath.MoveStepType.SWIM);
        } else if (gear == GEAR_RAM) {
            cmd.findPathTo(dest, MovePath.MoveStepType.FORWARDS);
        } else if (gear == GEAR_IMMEL) {
            cmd.addStep(MovePath.MoveStepType.UP, true, true);
            cmd.addStep(MovePath.MoveStepType.UP, true, true);
            cmd.addStep(MovePath.MoveStepType.DEC, true, true);
            cmd.addStep(MovePath.MoveStepType.DEC, true, true);
            cmd.rotatePathfinder(cmd.getFinalCoords().direction(dest), true);
            gear = GEAR_LAND;
        } else if (gear == GEAR_SPLIT_S) {
            cmd.addStep(MovePath.MoveStepType.DOWN, true, true);
            cmd.addStep(MovePath.MoveStepType.DOWN, true, true);
            cmd.addStep(MovePath.MoveStepType.ACC, true, true);
            cmd.rotatePathfinder(cmd.getFinalCoords().direction(dest), true);
            gear = GEAR_LAND;
        }
        if (gear == GEAR_LONGEST_WALK || gear == GEAR_LONGEST_RUN) {
            ...
            if (gear == GEAR_LONGEST_WALK) {
                maxMp = ce().getWalkMP();
                stepType = MovePath.MoveStepType.BACKWARDS;
                gear = GEAR_BACKUP;
            } else {
                maxMp = ce().getRunMPwithoutMASC();
                stepType = MovePath.MoveStepType.FORWARDS;
                gear = GEAR_LAND;
            }
    ...
    public synchronized void hexMoused(BoardViewEvent b) {
        ...
        if ((b.getType() == BoardViewEvent.BOARD_HEX_DRAGGED) && !nopath) {
            ...
            if (gear == MovementDisplay.GEAR_RAM) {
                ...
                cmd.addStep(MovePath.MoveStepType.RAM);
    ...
    private void updateSearchlightButton() {
        ...
        setSearchlightEnabled(
                ce.hasSpotlight() && !cmd.contains(MovePath.MoveStepType.SEARCHLIGHT),
                ce().isUsingSpotlight());
    }
    ...
    private void updateRollButton() {
        ...
        if (cmd.contains(MovePath.MoveStepType.ROLL)) {
            setRollEnabled(false);
        }
    ...
    private void updateHoverButton() {
        ...
        if (!cmd.contains(MovePath.MoveStepType.HOVER)) {
    ...
    private synchronized void updateSpeedButtons() {
        ...
        } else if (last.getType() == MovePath.MoveStepType.ACC) {
            setAccEnabled(true);
        } else if ((last.getType() == MovePath.MoveStepType.DEC) && (vel > 0)) {
            setDecEnabled(true);
        }
        ...
        setAccNEnabled(false);
        setDecNEnabled(false);
        if (!cmd.contains(MovePath.MoveStepType.ACC) && !cmd.contains(MovePath.MoveStepType.DEC)
            && !cmd.contains(MovePath.MoveStepType.DECN)) {
            setAccNEnabled(true);
        }

        if (!cmd.contains(MovePath.MoveStepType.ACC) && !cmd.contains(MovePath.MoveStepType.DEC)
            && !cmd.contains(MovePath.MoveStepType.ACCN) && (veln > 0)) {
            setDecNEnabled(true);
        }
    ...
    private void updateConvertModeButton() {
        ...
        if (cmd.length() > 0 && cmd.getLastStep().getType() != MovePath.MoveStepType.CONVERT_MODE) {
            setModeConvertEnabled(false);
            return;
        }
    ...
    private void updateManeuverButton() {
        ...
        if ((null != cmd) && cmd.contains(MovePath.MoveStepType.MANEUVER)) {
            setManeuverEnabled(false);
        } else {
    ...
    private void updateLayMineButton() {
        ...
        if (!ce.canLayMine() || cmd.contains(MovePath.MoveStepType.LAY_MINE)) {
    ...
    private boolean addManeuver(int type) {
        ...
        switch (type) {
            case (ManeuverType.MAN_HAMMERHEAD):
                cmd.addStep(MovePath.MoveStepType.YAW, true, true);
                return true;
            case (ManeuverType.MAN_HALF_ROLL):
                cmd.addStep(MovePath.MoveStepType.ROLL, true, true);
                return true;
            ...
            case (ManeuverType.MAN_VIFF):
                ...
                cmd.addStep(MovePath.MoveStepType.UP);
                return true;
            case (ManeuverType.MAN_SIDE_SLIP_LEFT):
                ...
                if (clientgui.getClient().getGame().getBoard().getType() == Board.T_GROUND) {
                    for (int i = 0; i < 8; i++) {
                        cmd.addStep(MovePath.MoveStepType.LATERAL_LEFT, true, true);
                    }
                    for (int i = 0; i < 8; i++) {
                        cmd.addStep(MovePath.MoveStepType.FORWARDS, true, true);
                    }
                } else {
                    cmd.addStep(MovePath.MoveStepType.LATERAL_LEFT, true, true);
                }
            ...
            case (ManeuverType.MAN_SIDE_SLIP_RIGHT):
                ...
                if (clientgui.getClient().getGame().getBoard().getType() == Board.T_GROUND) {
                    for (int i = 0; i < 8; i++) {
                        cmd.addStep(MovePath.MoveStepType.LATERAL_RIGHT, true, true);
                    }
                    for (int i = 0; i < 8; i++) {
                        cmd.addStep(MovePath.MoveStepType.FORWARDS, true, true);
                    }
            ...
            case (ManeuverType.MAN_LOOP):
                cmd.addStep(MovePath.MoveStepType.LOOP, true, true);
                return true;
            default:
                return false;
        }
    ...
    public void computeMovementEnvelope(Entity suggestion) {
        ...
        MovePath.MoveStepType stepType = (mvMode == GEAR_BACKUP) ? MovePath.MoveStepType.BACKWARDS
                : MovePath.MoveStepType.FORWARDS;
        if (mvMode == GEAR_JUMP || mvMode == GEAR_DFA) {
            mp.addStep(MovePath.MoveStepType.START_JUMP);
        }
    ...
    public void computeModifierEnvelope() {
        ...
        MovePath.MoveStepType stepType = (gear == GEAR_BACKUP) ? MovePath.MoveStepType.BACKWARDS
                                                      : MovePath.MoveStepType.FORWARDS;
    ...
    public synchronized void actionPerformed(ActionEvent ev) {
        ...
        if (actionCmd.equals(MoveCommand.MOVE_NEXT.getCmd())) {
            ...
            if ((gear == MovementDisplay.GEAR_JUMP)
                ...
                cmd.addStep(MovePath.MoveStepType.UNJAM_RAC);
                ready();
            ...
            cmd.addStep(MovePath.MoveStepType.SEARCHLIGHT);
            ...
            if ((null != mf) && clientgui.doYesNoDialog(title, msg)) {
                cmd.addStep(MovePath.MoveStepType.CLEAR_MINEFIELD, mf);
                ready();
            }
            ...
            if (!cmd.contains(MovePath.MoveStepType.HULL_DOWN)) {
                clear();
            }
            ...
            if (opts.booleanOption(OptionsConstants.ADVGRNDMOV_TACOPS_CAREFUL_STAND)
                ...
                if (response.getAnswer()) {
                    ...
                    if (cmd.getFinalProne() || cmd.getFinalHullDown()) {
                        cmd.addStep(MovePath.MoveStepType.CAREFUL_STAND);
                    }
                } else {
                    if (cmd.getFinalProne() || cmd.getFinalHullDown()) {
                        cmd.addStep(MovePath.MoveStepType.GET_UP);
                    }
                }
            } else {
                butDone.setText("<html><b>" + Messages.getString("MovementDisplay.Move") + "</b></html"); //$NON-NLS-1$
            ...
            gear = MovementDisplay.GEAR_LAND;
            if (!cmd.getFinalProne()) {
                cmd.addStep(MovePath.MoveStepType.GO_PRONE);
            }
            clientgui.bv.drawMovementData(ce(), cmd);
            butDone.setText("<html><b>" + Messages.getString("MovementDisplay.Move") + "</b></html>"); //$NON-NLS-1$
            ...
            if (!cmd.getFinalHullDown()) {
                cmd.addStep(MovePath.MoveStepType.HULL_DOWN);
            }
            clientgui.bv.drawMovementData(ce(), cmd);
            butDone.setText("<html><b>" + Messages.getString("MovementDisplay.Move") + "</b></html>"); //$NON-NLS-1$
            ...
            // $NON-NLS-2$
            clear();
            cmd.addStep(MovePath.MoveStepType.FLEE);
            ready();
            ...
            if (opts.booleanOption(OptionsConstants.ADVAERORULES_RETURN_FLYOVER)
                ...
                cmd.addStep(MovePath.MoveStepType.RETURN);
            } else {
                cmd.addStep(MovePath.MoveStepType.OFF);
            }
            ready();
            ...
            if (ce instanceof Tank) {
                if (clientgui
                    ...
                    // $NON-NLS-2$
                    clear();
                    cmd.addStep(MovePath.MoveStepType.EJECT);
                    ready();
                }
                ...
                if (clientgui
                    ...
                    if (!ce.isSpaceborne() && ce.getAltitude() == 0) {
                        ...
                        cmd.addStep(MovePath.MoveStepType.EJECT, ce, pos);
                    } else {
                        cmd.addStep(MovePath.MoveStepType.EJECT);
                    }
                    ready();
            ...
            if (other != null) {
                cmd.addStep(MovePath.MoveStepType.LOAD);
                clientgui.bv.drawMovementData(ce(), cmd);
                gear = MovementDisplay.GEAR_LAND;
            ...
            if (other != null) {
                cmd.addStep(MovePath.MoveStepType.TOW);
                clientgui.bv.drawMovementData(ce(), cmd);
            ...
            if (other != null) {
                cmd.addStep(MovePath.MoveStepType.DISCONNECT, other);
                clientgui.bv.drawMovementData(ce(), cmd);
            } // else - didn't find a unit to tow
        } else if (actionCmd.equals(MoveCommand.MOVE_MOUNT.getCmd())) {
            Entity other = getMountedUnit();
            if (other != null) {
                cmd.addStep(MovePath.MoveStepType.MOUNT, other);
                ready();
            }
            ...
            if (other != null) {
                if (ce() instanceof SmallCraft 
                    ...
                    if (null != pos) {
                        ...
                        cmd.addStep(MovePath.MoveStepType.UNLOAD, other, pos);
                        clientgui.bv.drawMovementData(ce(), cmd);
                        ready();
                    }
                } else {
                    ...
                    // unloading
                    cmd.addStep(MovePath.MoveStepType.UNLOAD, other);
                    clientgui.bv.drawMovementData(ce(), cmd);
                }
            ...
            if (ce.isAero()
                ...
                cmd.addStep(MovePath.MoveStepType.ACC, true);
            }
            cmd.addStep(MovePath.MoveStepType.DOWN);
            clientgui.bv.drawMovementData(ce(), cmd);
        } else if (actionCmd.equals(MoveCommand.MOVE_CLIMB_MODE.getCmd())) {
            MoveStep ms = cmd.getLastStep();
            if ((ms != null)
                    && ((ms.getType() == MovePath.MoveStepType.CLIMB_MODE_ON) || (ms
                            .getType() == MovePath.MoveStepType.CLIMB_MODE_OFF))) {
                MoveStep lastStep = cmd.getLastStep();
                cmd.removeLastStep();
                ...
                if (lastStep.getType() == MovePath.MoveStepType.CLIMB_MODE_ON) {
                    cmd.addStep(MovePath.MoveStepType.CLIMB_MODE_OFF);
                } else {
                    cmd.addStep(MovePath.MoveStepType.CLIMB_MODE_ON);
                }
            ...
            if (i != -1) {
                ...
                cmd.addStep(MovePath.MoveStepType.LAY_MINE, i);
                clientgui.bv.drawMovementData(ce, cmd);
            }
            ...
            cmd.addStep(MovePath.MoveStepType.DIG_IN);
            clientgui.bv.drawMovementData(ce(), cmd);
        } else if (actionCmd.equals(MoveCommand.MOVE_FORTIFY.getCmd())) {
            cmd.addStep(MovePath.MoveStepType.FORTIFY);
            clientgui.bv.drawMovementData(ce(), cmd);
        } else if (actionCmd.equals(MoveCommand.MOVE_TAKE_COVER.getCmd())) {
            cmd.addStep(MovePath.MoveStepType.TAKE_COVER);
            clientgui.bv.drawMovementData(ce(), cmd);
        } else if (actionCmd.equals(MoveCommand.MOVE_SHAKE_OFF.getCmd())) {
            cmd.addStep(MovePath.MoveStepType.SHAKE_OFF_SWARMERS);
            clientgui.bv.drawMovementData(ce(), cmd);
            ...
            if (clientgui
                ...
                cmd.addStep(MovePath.MoveStepType.SHUTDOWN);
                ready();
            }
            ...
            if (clientgui
                ...
                clear();
                cmd.addStep(MovePath.MoveStepType.STARTUP);
                ready();
            }
            ...
            if (clientgui
                ...
                // $NON-NLS-2$
                cmd.addStep(MovePath.MoveStepType.SELF_DESTRUCT);
                ready();
            }
            ...
            cmd.addStep(MovePath.MoveStepType.ROLL);
            clientgui.bv.drawMovementData(ce, cmd);
        } else if (actionCmd.equals(MoveCommand.MOVE_HOVER.getCmd())) {
            cmd.addStep(MovePath.MoveStepType.HOVER);
            if (ce instanceof LandAirMech
            ...
            if (!undocked.isEmpty()) {
                cmd.addStep(MovePath.MoveStepType.UNDOCK, undocked);
            }
            TreeMap<Integer, Vector<Integer>> launched = getLaunchedUnits();
            if (!launched.isEmpty()) {
                cmd.addStep(MovePath.MoveStepType.LAUNCH, launched);
            }
            ...
            if (recoverer != -1) {
                cmd.addStep(MovePath.MoveStepType.RECOVER, recoverer, -1);
                clientgui.bv.drawMovementData(ce, cmd);
            }
            ...
            if (!dropped.isEmpty()) {
                cmd.addStep(MovePath.MoveStepType.DROP, dropped);
                clientgui.bv.drawMovementData(ce, cmd);
            }
            ...
            if (ce().isAero()
                ...
                if (clientgui
                    ...
                    // $NON-NLS-2$
                    clear();
                    cmd.addStep(MovePath.MoveStepType.TAKEOFF);
                    ready();
                }
            }
            ...
            if (clientgui
                ...
                clear();
                cmd.addStep(MovePath.MoveStepType.VTAKEOFF);
                ready();
            }
            ...
            if (ce().isAero()
                ...
                if (clientgui
                    ...
                    clear();
                    cmd.addStep(MovePath.MoveStepType.LAND);
                    ready();
                }
            }
            ...
            if (ce().isAero()
                ...
                if (clientgui
                        .doYesNoDialog(
                                ...
                                Messages.getString("MovementDisplay.LandDialog.message"))) { //$NON-NLS-1$ //$NON-NLS-2$
                    clear();
                    cmd.addStep(MovePath.MoveStepType.VLAND);
                    ready();
                }
            }
    ...
    public void FieldofFire(Entity unit, int[][] ranges, int arc, int loc) {
        ...
        if (!(clientgui.getClient().getGame().getPhase() == IGame.Phase.PHASE_MOVEMENT)) return;
