...
import org.redbat.roguetech.megamek.client.bot.ChatProcessor;
import org.redbat.roguetech.megamek.client.bot.PhysicalCalculator;
import org.redbat.roguetech.megamek.client.bot.PhysicalOption;
import org.redbat.roguetech.megamek.client.bot.princess.PathRanker.PathRankerType;
import org.redbat.roguetech.megamek.client.bot.princess.UnitBehavior.BehaviorType;
import org.redbat.roguetech.megamek.client.ui.SharedUtility;
import org.redbat.roguetech.megamek.common.*;
import org.redbat.roguetech.megamek.common.actions.EntityAction;
import org.redbat.roguetech.megamek.common.actions.SearchlightAttackAction;
import org.redbat.roguetech.megamek.common.annotations.Nullable;
import org.redbat.roguetech.megamek.common.containers.PlayerIDandList;
import org.redbat.roguetech.megamek.common.event.GameCFREvent;
import org.redbat.roguetech.megamek.common.event.GamePlayerChatEvent;
...
import org.redbat.roguetech.megamek.common.net.Packet;
...
import org.redbat.roguetech.megamek.common.pathfinder.PathDecorator;
import org.redbat.roguetech.megamek.common.util.BoardUtilities;
import org.redbat.roguetech.megamek.common.util.StringUtil;
...
public class Princess extends BotClient {
    ...
    FireControl getFireControl(Entity entity) {
        if(entity.hasETypeFlag(Entity.ETYPE_INFANTRY)) {
            return fireControls.get(FireControl.FireControlType.Infantry);
        ...
        // so let's get them doing that.
            ...
            return fireControls.get(FireControl.FireControlType.MultiTarget);
        }
        
        return fireControls.get(FireControl.FireControlType.Basic);
    }
    ...
    public void initializeFireControls() {
        ...
        fireControls.put(FireControl.FireControlType.Basic, fireControl);
        
        InfantryFireControl infantryFireControl = new InfantryFireControl(this);
        fireControls.put(FireControl.FireControlType.Infantry, infantryFireControl);
        
        MultiTargetFireControl multiTargetFireControl = new MultiTargetFireControl(this);
        fireControls.put(FireControl.FireControlType.MultiTarget, multiTargetFireControl);
    }
    ...
    private void unjamRAC(MovePath path) { 
        if(path.getEntity().canUnjamRAC() && 
                ...
                !path.isJumping()) {
            path.addStep(MovePath.MoveStepType.UNJAM_RAC);
        }
    }
    ...
    private void evadeIfNotFiring(MovePath path, boolean possibleToInflictDamage) {
        ...
        if(pathEntity.isAirborne() &&
           ...
           (path.getMpUsed() <= AeroPathUtil.calculateMaxSafeThrust((IAero) path.getEntity()) - 2)) {
            path.addStep(MovePath.MoveStepType.EVADE);
        }
    }
    ...
    private void turnOnSearchLight(MovePath path, boolean possibleToInflictDamage) {
        ...
        if(possibleToInflictDamage &&
            ...
            path.addStep(MovePath.MoveStepType.SEARCHLIGHT);
        }
    }
        ...
        for(Transporter transport : movingEntity.getTransports()) {
            for(Entity loadedEntity : transport.getLoadedUnits()) {
                ...
                if(!unloadFatal && !unloadIllegal && inEngagementRange) {
                    path.addStep(MovePath.MoveStepType.UNLOAD, loadedEntity, pathEndpoint);
