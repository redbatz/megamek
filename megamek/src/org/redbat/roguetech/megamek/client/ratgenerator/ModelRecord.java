	...
	public ModelRecord(MechSummary ms) {
    	...
    	for (int i = 0; i < ms.getEquipmentNames().size(); i++) {
    		...
    		if (eq instanceof WeaponType) {
        		...
        		if (eq instanceof MGWeapon ||
        				eq instanceof BPodWeapon) {
        			apWeapons = true;
        		}
        		if (((WeaponType) eq).getAmmoType() > AmmoType.T_NA) {
    	...
    	clan = ms.isClan();
    	if (Engine.getEngineTypeByString(ms.getEngineName()) == Engine.XL_ENGINE
