...
public class LandAirMech extends BipedMech implements IAero, IBomber {
    ...
    public LandAirMech(int inGyroType, int inCockpitType, int inLAMType) {
        ...
        setCritical(LOC_HEAD, 3, new CriticalSlot(CriticalSlot.TYPE_SYSTEM, LAM_AVIONICS));
        setCritical(LOC_LT, 1, new CriticalSlot(CriticalSlot.TYPE_SYSTEM, LAM_AVIONICS));
        setCritical(LOC_RT, 1, new CriticalSlot(CriticalSlot.TYPE_SYSTEM, LAM_AVIONICS));
        setCritical(LOC_LT, 0, new CriticalSlot(CriticalSlot.TYPE_SYSTEM, LAM_LANDING_GEAR));
        setCritical(LOC_RT, 0, new CriticalSlot(CriticalSlot.TYPE_SYSTEM, LAM_LANDING_GEAR));
        for (int i = 0; i < getNumberOfCriticals(LOC_CT); i++) {
            if (null == getCritical(LOC_CT, i)) {
                setCritical(LOC_CT, i, new CriticalSlot(CriticalSlot.TYPE_SYSTEM, LAM_LANDING_GEAR));
                break;
            }
        }
    ...
    public int getAirMechCruiseMP(boolean gravity, boolean ignoremodulararmor) {
        if (game != null && game.getBoard().inAtmosphere()
                && (isLocationBad(LOC_LT) || isLocationBad(LOC_RT))) {
            return 0;
        }
    ...
    public PilotingRollData addEntityBonuses(PilotingRollData roll) {
        ...
        if ((getCockpitType() == COCKPIT_SMALL)
                && !hasAbility(OptionsConstants.MD_BVDNI)) {
    ...
    public boolean canConvertTo(int fromMode, int toMode) {
        ...
        if ((toMode == CONV_MODE_MECH || fromMode == CONV_MODE_MECH)
                && (getBadCriticals(CriticalSlot.TYPE_SYSTEM, ACTUATOR_SHOULDER, LOC_RARM)
                        + getBadCriticals(CriticalSlot.TYPE_SYSTEM, ACTUATOR_UPPER_ARM, LOC_RARM)
                        + getBadCriticals(CriticalSlot.TYPE_SYSTEM, ACTUATOR_LOWER_ARM, LOC_RARM)
                        + getBadCriticals(CriticalSlot.TYPE_SYSTEM, ACTUATOR_SHOULDER, LOC_LARM)
                        + getBadCriticals(CriticalSlot.TYPE_SYSTEM, ACTUATOR_UPPER_ARM, LOC_LARM)
                        + getBadCriticals(CriticalSlot.TYPE_SYSTEM, ACTUATOR_LOWER_ARM, LOC_LARM) > 0)) {
            return false;
        }
        ...
        if ((toMode == CONV_MODE_FIGHTER || fromMode == CONV_MODE_FIGHTER)
                && (getBadCriticals(CriticalSlot.TYPE_SYSTEM, ACTUATOR_HIP, LOC_RLEG)
                        + getBadCriticals(CriticalSlot.TYPE_SYSTEM, ACTUATOR_UPPER_LEG, LOC_RLEG)
                        + getBadCriticals(CriticalSlot.TYPE_SYSTEM, ACTUATOR_LOWER_LEG, LOC_RLEG)
                        + getBadCriticals(CriticalSlot.TYPE_SYSTEM, ACTUATOR_HIP, LOC_LLEG)
                        + getBadCriticals(CriticalSlot.TYPE_SYSTEM, ACTUATOR_UPPER_LEG, LOC_LLEG)
                        + getBadCriticals(CriticalSlot.TYPE_SYSTEM, ACTUATOR_LOWER_LEG, LOC_LLEG) > 0)) {
            return false;
        }
    ...
    public Map<String, Integer> groupWeaponsByLocation() {
        ...
        for (Mounted mounted : getTotalWeaponList()) {
            ...
            if (mounted.isRearMounted() || (loc == LOC_LLEG) || (loc == LOC_RLEG)) {
                loc = LOC_CAPITAL_AFT;
            }
    ...
    public long getEntityType() {
        return ETYPE_MECH | ETYPE_BIPED_MECH | ETYPE_LAND_AIR_MECH;
    }
