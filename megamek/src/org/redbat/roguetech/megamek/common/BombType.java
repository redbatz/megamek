...
public class BombType extends AmmoType {
    ...
    public static void initializeTypes() {
        addType(BombType.createHighExplosiveBomb());
        addType(BombType.createClusterBomb());
        addType(BombType.createLaserGuidedBomb());
//        EquipmentType.addType(BombType.createCLLaserGuidedBomb());
        addType(BombType.createRocketBomb());
        addType(BombType.createTAGBomb());
//        EquipmentType.addType(BombType.createCLTAGBomb());
        addType(BombType.createAAAMissileBomb());
//        EquipmentType.addType(BombType.createCLAAAMissileBomb());
        addType(BombType.createASMissileBomb());
//        EquipmentType.addType(BombType.createCLASMissileBomb());
        addType(BombType.createISASEWMissileBomb());
//        EquipmentType.addType(BombType.createCLASEWMissileBomb());
        addType(BombType.createArrowIVBomb());
//        EquipmentType.addType(BombType.createCLArrowIVBomb());
        addType(BombType.createArrowIVHomingBomb());
//        EquipmentType.addType(BombType.createCLArrowIVHomingBomb());
        addType(BombType.createInfernoBomb());
        addType(BombType.createLAAMissileBomb());
//        EquipmentType.addType(BombType.createCLLAAMissileBomb());
        addType(BombType.createThunderBomb());
        addType(BombType.createTorpedoBomb());
        addType(BombType.createAlamoBomb());
        addType(BombType.createSmallFuelAirBomb());
        addType(BombType.createLargeFuelAirBomb());
    }
	...
	private static BombType createAAAMissileBomb() {
		...
		bomb.flags = bomb.flags.or(F_OTHER_BOMB);
		bomb.rackSize = 1;
		bomb.ammoType = T_AAA_MISSILE;
	...
	private static BombType createArrowIVHomingBomb() {
		...
		bomb.rackSize = 20;
		bomb.ammoType = T_ARROW_IV_BOMB;
		bomb.bombType = BombType.B_HOMING;
		bomb.munitionType = M_HOMING;
		...
		bomb.flags = bomb.flags.or(F_SPACE_BOMB);
		bomb.shots = 1;
		bomb.bv = 0;
		bomb.cost = 3000;
		bomb.rulesRefs = "358,TO";
	...
	private static BombType createClusterBomb() {
		...
		bomb.rackSize = 1;
		bomb.ammoType = T_BOMB;
		bomb.bombType = BombType.B_CLUSTER;
		bomb.flags = bomb.flags.or(F_SPACE_BOMB).or(F_GROUND_BOMB);
		bomb.shots = 1;
		bomb.bv = 13;
		bomb.cost = 8000;
	...
	private static BombType createSmallFuelAirBomb() {
        ...
        bomb.flags = bomb.flags.or(F_GROUND_BOMB);
        bomb.shots = 1;
        bomb.bv = 37;
        bomb.cost = 18000;
        bomb.tonnage = .5;
	...
	private static BombType createAlamoBomb() {
		...
		bomb.damagePerShot = 10;
		bomb.rackSize = 1;
		bomb.ammoType = T_ALAMO;
		bomb.bombType = BombType.B_ALAMO;
		bomb.shots = 1;
		bomb.bv = 100;
		bomb.cost = 1000000;
		bomb.flags = bomb.flags.or(F_NUCLEAR).or(F_OTHER_BOMB);
		bomb.capital = true;
		bomb.techAdvancement.setTechBase(TECH_BASE_IS).setTechRating(RATING_E)
