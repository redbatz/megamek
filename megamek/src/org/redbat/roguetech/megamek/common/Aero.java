...
public class Aero extends Entity implements IAero, IBomber {
    ...
    public int calculateBattleValue(boolean ignoreC3, boolean ignorePilot) {
        ...
        for (Mounted mounted : getAmmo()) {
            ...
            if (mounted.getLocation() == LOC_NONE) {
    ...
    public Vector<Report> victoryReport() {
        ...
        if (((getEntityType() & ETYPE_DROPSHIP) == 0) || ((getEntityType() & ETYPE_SMALL_CRAFT) == 0)
                || ((getEntityType() & ETYPE_FIGHTER_SQUADRON) == 0)
                || ((getEntityType() & ETYPE_JUMPSHIP) == 0)
                || ((getEntityType() & ETYPE_SPACE_STATION) == 0)) {
            r = new Report(7036);
        } else {
            r = new Report(7030);
        }
    ...
    public void updateSensorOptions() {
        ...
        if (!isSpaceborne()) {
            ...
            if (hasETypeFlag(ETYPE_DROPSHIP)) {
                for (Sensor sensor : getSensors()) {
                    if (sensor.getType() == Sensor.TYPE_SPACECRAFT_ESM) {
                        hasESM = false;
            ...
            } else if (hasETypeFlag(ETYPE_AERO)) {
                for (Sensor sensor : getSensors()) {
                    if (sensor.getType() == Sensor.TYPE_AERO_THERMAL) {
        ...
        if (isSpaceborne()) {
            if (hasETypeFlag(ETYPE_DROPSHIP)
                    || hasETypeFlag(ETYPE_SPACE_STATION)
                    || hasETypeFlag(ETYPE_JUMPSHIP)
                    || hasETypeFlag(ETYPE_WARSHIP)) {
                ...
                if (getDesignType() == Aero.MILITARY) {
                    if (!hasESM) {
                        ...
                        || hasETypeFlag(ETYPE_SMALL_CRAFT)) {
