...
public class TestTank extends TestEntity {
    ...
    public double getTankWeightTurret() {
        ...
        if (tank.isSupportVehicle()) {
            if (getEntity().getWeight() < 5) {
                return ceil(weight, Ceil.KILO);
            } else {
                return ceil(weight, Ceil.HALFTON);
            }
        } else {
            return ceilMaxHalf(weight, getWeightCeilingTurret());
        }
    }
    ...
    public double getTankWeightLifting() {
        switch (tank.getMovementMode()) {
            ...
            case SUBMARINE:
            case WIGE:
                return ceilMaxHalf(tank.getWeight() / 10.0f,
    ...
    public double getWeightControls() {
        if (tank.hasNoControlSystems()) {
            ...
            return ceilMaxHalf(tank.getWeight() / 20.0f,
    ...
    public String printWeightMisc() {
        ...
        String dualTurretString = !tank.hasNoDualTurret() ? StringUtil.makeLength("Front Turret:",
                getPrintSize() - 5)
                + makeWeightString(getTankWeightDualTurret()) + "\n" : "";
        return turretString + dualTurretString
                ...
                + (getWeightPowerAmp() != 0 ? StringUtil.makeLength(
                        ...
                        + makeWeightString(getWeightPowerAmp()) + "\n" : "");
    }
    ...
    public String printWeightCarryingSpace() {
        if (tank.getExtraCrewSeats() > 0) {
            return super.printWeightCarryingSpace()
                    ...
                    + makeWeightString(tank.getExtraCrewSeats() * 0.5) + "\n";
        } else {
            return super.printWeightCarryingSpace();
        }
    }
    ...
    public double getWeightPowerAmp() {
        ...
        if (!engine.isFusion() && (engine.getEngineType() != Engine.FISSION)) {
            ...
            return ceil(weight / 10, getWeightCeilingPowerAmp());
        }
        return 0;
    }
    ...
    public boolean hasIllegalEquipmentCombinations(StringBuffer buff) {
        ...
        if (tank.isOmni()) {
            ...
            if (tank.isSupportVehicle()) {
                if (getEntity().getWeight() < 5) {
                    turretWeight = ceil(turretWeight, Ceil.KILO);
                    turret2Weight = ceil(turret2Weight, Ceil.KILO);
                } else {
                    turretWeight = ceil(turretWeight, Ceil.HALFTON);
                    turret2Weight = ceil(turret2Weight, Ceil.HALFTON);
                }
            } else {
                turretWeight = ceil(turretWeight,
                        getWeightCeilingTurret());
                turret2Weight = ceil(turret2Weight,
                        getWeightCeilingTurret());
            }
