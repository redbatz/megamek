...
public class BipedMech extends Mech {
    ...
    public boolean hasActiveShield(int location, boolean rear) {
        ...
        switch (location) {
            ...
            case LOC_HEAD:
                ...
                if (hasActiveShield(LOC_LARM) || hasActiveShield(LOC_RARM)) {
                    return true;
                }
                // else
                return false;
    ...
    public boolean hasFunctionalLegAES() {
        ...
        for (Mounted mounted : getMisc()) {
            if ((mounted.getLocation() == LOC_LLEG) || (mounted.getLocation() == LOC_RLEG)) {
    ...
    public boolean canGoHullDown() {
        ...
        // check the locations
        retVal = (!isLocationBad(LOC_LLEG) && !isLocationBad(LOC_RLEG) && !isLocationDoomed(LOC_LLEG)
                  && !isLocationDoomed(LOC_RLEG));
        if (!retVal) {
            return false;
        }
        // check the Gyro
    ...
    public long getEntityType() {
        return ETYPE_MECH | ETYPE_BIPED_MECH;
    }
    ...
    public boolean canZweihander() {
        return (getCrew() != null)
                ...
                && hasWorkingSystem(ACTUATOR_HAND, LOC_RARM)
                && hasWorkingSystem(ACTUATOR_HAND, LOC_LARM)
                && !isLocationBad(LOC_RARM)
                && !isLocationBad(LOC_LARM)
                && !isProne();
    }
