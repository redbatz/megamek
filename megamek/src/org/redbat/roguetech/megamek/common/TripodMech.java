...
public class TripodMech extends Mech {
    ...
    public TripodMech() {
        this(GYRO_STANDARD, COCKPIT_STANDARD);
    }
    ...
    public long getEntityType() {
        return ETYPE_MECH | ETYPE_TRIPOD_MECH;
    }
    ...
    public boolean canFlipArms() {
        ...
        if (hasSystem(ACTUATOR_HAND, LOC_LARM)) {
            canFlip = false;
        } else if (hasSystem(ACTUATOR_LOWER_ARM, LOC_LARM)) {
            canFlip = false;
        } else if (hasSystem(ACTUATOR_HAND, LOC_RARM)) {
            canFlip = false;
        } else if (hasSystem(ACTUATOR_LOWER_ARM, LOC_RARM)) {
            canFlip = false;
        }
    ...
    public PilotingRollData addEntityBonuses(PilotingRollData roll) {
        ...
        locsToCheck[0] = LOC_RLEG;
        locsToCheck[1] = LOC_LLEG;
        locsToCheck[2] = LOC_CLEG;
    ...
    protected double getArmActuatorCost() {
        ...
        if (hasSystem(ACTUATOR_LOWER_ARM, LOC_LARM)) {
            numOfLowerArmActuators++;
        }
        if (hasSystem(ACTUATOR_UPPER_ARM, LOC_LARM)) {
            numOfUpperArmActuators++;
        }
        if (hasSystem(ACTUATOR_HAND, LOC_RARM)) {
            numOfHands++;
        }
        if (hasSystem(ACTUATOR_LOWER_ARM, LOC_RARM)) {
            numOfLowerArmActuators++;
        }
        if (hasSystem(ACTUATOR_UPPER_ARM, LOC_RARM)) {
            numOfUpperArmActuators++;
        }
    ...
    public boolean hasClaw(int location) {
        ...
        if ((location != LOC_RARM) && (location != LOC_LARM)) {
            return false;
        }
    ...
    public boolean hasVibroblades() {
        ...
        if (hasVibrobladesInLocation(LOC_RARM)) {
            count++;
        }
        if (hasVibrobladesInLocation(LOC_LARM)) {
            count++;
        }
    ...
    public boolean hasShield() {
        for (Mounted m : getMisc()) {
            EquipmentType type = m.getType();
            if (((m.getLocation() == LOC_LARM) || (m.getLocation() == LOC_RARM))
    ...
    public int getNumberOfShields(long size) {
        ...
        for (Mounted m : getMisc()) {
            ...
            if ((type instanceof MiscType) && type.hasFlag(MiscType.F_CLUB)
                ...
                if (m.getLocation() == LOC_RARM) {
                    raShield = 1;
                }
                if (m.getLocation() == LOC_LARM) {
                    laShield = 1;
                }
                // break now.
    ...
    public boolean hasActiveShield(int location, boolean rear) {
        ...
        switch (location) {
            ...
            case LOC_HEAD:
                ...
                if (hasActiveShield(LOC_LARM)
                    || hasActiveShield(LOC_RARM)) {
                    return true;
                }
                // else
                return false;
            ...
            case LOC_LT:
            case LOC_LLEG:
                return hasActiveShield(LOC_LARM);
            default:
                return hasActiveShield(LOC_RARM);
        }
    }
    ...
    public boolean hasPassiveShield(int location, boolean rear) {
        ...
        switch (location) {
            ...
            case LOC_LT:
                ...
                return hasPassiveShield(LOC_LARM);
            // RA RT
            default:
                if (rear) {
                    // that
                ...
                return hasPassiveShield(LOC_RARM);
        }
    }
    ...
    public boolean hasFunctionalLegAES() {
        ...
        for (Mounted mounted : getMisc()) {
            if ((mounted.getLocation() == LOC_LLEG)
                || (mounted.getLocation() == LOC_RLEG)
                || (mounted.getLocation() == LOC_CLEG)) {
                if (((MiscType) mounted.getType())
                    ...
                    if (mounted.getLocation() == LOC_LLEG) {
                        leftLeg = true;
                    } else if (mounted.getLocation() == LOC_RLEG) {
                        rightLeg = true;
                    } else {
                        centerLeg = true;
                    }
    ...
    public boolean canGoHullDown() {
        ...
        // check the locations
        retVal = (!isLocationBad(LOC_LLEG)
                  && !isLocationBad(LOC_RLEG) && !isLocationBad(LOC_CLEG)
                  && !isLocationDoomed(LOC_LLEG) && !isLocationDoomed(LOC_RLEG))
    ...
    public void addStandingPenalties(PilotingRollData roll) {
        ...
        if (game.getOptions().booleanOption(OptionsConstants.ADVGRNDMOV_TACOPS_ATTEMPTING_STAND)) {
            ...
            locsToCheck[0] = LOC_RARM;
            locsToCheck[1] = LOC_LARM;
            ...
            for (int i = 0; i < locsToCheck.length; i++) {
                ...
                if (isLocationBad(loc)) {
                    ...
                    // check for damaged hip actuators
                    if (!hasWorkingSystem(ACTUATOR_HAND, loc)) {
                        roll.addModifier(1, getLocationName(loc)
                                            + " hand Actuator missing/destroyed");
                    } else if (!hasWorkingSystem(ACTUATOR_LOWER_ARM, loc)) {
                        roll.addModifier(1, getLocationName(loc)
                                            + " lower Actuator missing/destroyed");
                    } else if (!hasWorkingSystem(ACTUATOR_UPPER_ARM, loc)) {
                        roll.addModifier(1, getLocationName(loc)
                                            + " upper ctuator missing/destroyed");
                    } else if (!hasWorkingSystem(ACTUATOR_SHOULDER, loc)) {
                        roll.addModifier(1, getLocationName(loc)
            ...
            if (side == ToHitData.SIDE_FRONT) {
                ...
                switch (roll) {
                    case 2:
                        ...
                        return tac(table, side, LOC_CT, cover, false);
                    case 3:
                    case 4:
                        return new HitData(LOC_RARM);
                    case 5:
                    case 9:
                        int legRoll = Compute.d6();
                        if (legRoll <= 2) {
                            return new HitData(LOC_RLEG);
                        } else if (legRoll <= 4) {
                            return new HitData(LOC_CLEG);
                        } else {
                            return new HitData(LOC_LLEG);
                        }
                    case 6:
                        return new HitData(LOC_RT);
                    case 7:
                        return new HitData(LOC_CT);
                    case 8:
                        return new HitData(LOC_LT);
                    case 10:
                    case 11:
                        return new HitData(LOC_LARM);
                    case 12:
                        if (getCrew().hasEdgeRemaining()
                            ...
                            HitData result = rollHitLocation(table, side,
                                                             aimedLocation, aimingMode, cover);
                            result.setUndoneLocation(new HitData(LOC_HEAD));
                            return result;
                        } // if
                        return new HitData(LOC_HEAD);
                }
                ...
                switch (roll) {
                    case 2:
                        ...
                        return tac(table, side, LOC_LT, cover, false);
                    case 3:
                    case 6:
                    case 11:
                    ...
                    case 8:
                        if (game.getOptions().booleanOption(
                            ...
                            return new HitData(LOC_CT, true);
                        }
                        return new HitData(LOC_CT);
                    case 9:
                        if (game.getOptions().booleanOption(
                                OptionsConstants.ADVCOMBAT_TACOPS_ADVANCED_MECH_HIT_LOCATIONS)) {
                            return new HitData(LOC_RT, true);
                        }
                        return new HitData(LOC_RT);
                    case 10:
                        return new HitData(LOC_RARM);
                ...
                switch (roll) {
                    case 2:
                        ...
                        return tac(table, side, LOC_RT, cover, false);
                    case 3:
                    case 6:
                    case 11:
                        int legRoll = Compute.d6() - 1;
                    ...
                    case 9:
                        if (game.getOptions().booleanOption(
                                OptionsConstants.ADVCOMBAT_TACOPS_ADVANCED_MECH_HIT_LOCATIONS)) {
                            return new HitData(LOC_LT, true);
                        }
                        return new HitData(LOC_LT);
                    case 10:
                        return new HitData(LOC_LARM);
                ...
                if (game.getOptions().booleanOption(
                    ...
                    switch (roll) {
                        case 2:
                            ...
                            return tac(table, side, LOC_CT, cover, true);
                        case 3:
                            return new HitData(LOC_RARM, true);
                        case 4:
                        case 5:
                        case 9:
                        case 10:
                        ...
                        case 8:
                            return new HitData(LOC_LT, true);
                        case 11:
                            return new HitData(LOC_LARM, true);
                        case 12:
                            if (getCrew().hasEdgeRemaining()
                                && getCrew().getOptions().booleanOption(
                                    "edge_when_headhit")) {
                            ...
                            return new HitData(LOC_HEAD, true);
                    }
                } else {
                    switch (roll) {
                        case 2:
        ...
        if ((table == ToHitData.HIT_SWARM)
            ...
            switch (roll) {
                case 2:
                    ...
                    return new HitData(LOC_HEAD, false, effects);
                case 3:
                    return new HitData(LOC_CT, true, effects);
                case 4:
                    return new HitData(LOC_RT, true, effects);
                case 5:
                    return new HitData(LOC_RT, false, effects);
                case 6:
                    return new HitData(LOC_RARM, false, effects);
                case 7:
                    return new HitData(LOC_CT, false, effects);
                case 8:
                    return new HitData(LOC_LARM, false, effects);
                case 9:
                    return new HitData(LOC_LT, false, effects);
                case 10:
                    return new HitData(LOC_LT, true, effects);
                case 11:
                    return new HitData(LOC_CT, true, effects);
                case 12:
                    if (getCrew().hasEdgeRemaining()
