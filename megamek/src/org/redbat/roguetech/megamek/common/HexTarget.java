...
public class HexTarget implements Targetable {
    ...
    public HexTarget(Coords c, int nType) {
        m_coords = c;
        m_type = nType;
        m_bIgnite = (nType == TYPE_HEX_IGNITE);
    }
    ...
    public boolean isImmobile() {
        return ((m_type != TYPE_HEX_BOMB) && (m_type != TYPE_HEX_AERO_BOMB));
    }

    public String getDisplayName() {
        String name = "";
        switch (m_type) {
            case (TYPE_FLARE_DELIVER):
                name = Messages.getString("HexTarget.DeliverFlare");
                break;
            case (TYPE_MINEFIELD_DELIVER):
