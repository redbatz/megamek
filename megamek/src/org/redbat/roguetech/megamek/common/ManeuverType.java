...
public class ManeuverType {
    ...
    public static boolean canPerform(int type, int velocity, int altitude, 
        ...
        for (final MoveStep step : mp.getStepVector()){
            if (step.getType() == MovePath.MoveStepType.MANEUVER){
                return false;
            }
        }
                ...
                if (game.getBoard().getType() == Board.T_GROUND){
                    ...
                    for (int i = 0; i < 8; i++){
                        if (type == MAN_SIDE_SLIP_LEFT){
                            tmpMp.addStep(MovePath.MoveStepType.LATERAL_LEFT,true,true);
                        } else {
                            tmpMp.addStep(MovePath.MoveStepType.LATERAL_RIGHT,true,true);
                        }
                    }
                    for (int i = 0; i < 8; i++){
                        tmpMp.addStep(MovePath.MoveStepType.FORWARDS,true,true);
                    }                    
