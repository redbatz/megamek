...
public class BoardEdgePathFinder {
    ...
    private void matchFacingToPath(MovePath initialPath, MoveStep intersectionStep) {
        ...
        MovePath.MoveStepType turnDirection = leftTurnCount > rightTurnCount ? MovePath.MoveStepType.TURN_RIGHT : MovePath.MoveStepType.TURN_LEFT;
    ...
    public MovePath findPathToEdge(Entity entity, int destinationRegion) {
        ...
        if(entity.hasETypeFlag(Entity.ETYPE_INFANTRY)) {
            startPath.addStep(MovePath.MoveStepType.CLIMB_MODE_OFF);
        } else {
            startPath.addStep(MovePath.MoveStepType.CLIMB_MODE_ON);
        }
     ...
     */
    public @Nullable
    MovePath getLongestNonEdgePath(Coords coords) {
    ...
    protected List<MovePath> generateChildNodes(MovePath parentPath, Set<Coords> visitedCoords) {
        ...
        leftChild.addStep(MovePath.MoveStepType.TURN_LEFT);
        leftChild.addStep(MovePath.MoveStepType.FORWARDS);
        processChild(leftChild, children, visitedCoords);

        MovePath centerChild = parentPath.clone();
        centerChild.addStep(MovePath.MoveStepType.FORWARDS);
        processChild(centerChild, children, visitedCoords);

        MovePath rightChild = parentPath.clone();
        rightChild.addStep(MovePath.MoveStepType.TURN_RIGHT);
        rightChild.addStep(MovePath.MoveStepType.FORWARDS);
