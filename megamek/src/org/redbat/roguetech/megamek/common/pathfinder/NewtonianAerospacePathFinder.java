...
public class NewtonianAerospacePathFinder {
    ...
    protected Map<MovePathFinder.CoordsWithFacing, Integer> visitedCoords = new HashMap<>();
    // This is a list of all possible moves
    protected List<MovePath.MoveStepType> moves;
    ...
    protected void initializeMoveList() {
        moves = new ArrayList<>();
        moves.add(MovePath.MoveStepType.TURN_RIGHT);
        moves.add(MovePath.MoveStepType.TURN_LEFT);
        moves.add(MovePath.MoveStepType.THRUST);
    }
    ...
    protected List<MovePath> generateStartingPaths(MovePath startingEdge) {
        ...
        visitedCoords.put(new MovePathFinder.CoordsWithFacing(defaultPath), defaultPath.getMpUsed());
        startingPaths.add(defaultPath);
        
        MovePath reverseEdge = startingEdge.clone();
        reverseEdge.addStep(MovePath.MoveStepType.YAW);
        aerospacePaths.add(reverseEdge);
        visitedCoords.put(new MovePathFinder.CoordsWithFacing(reverseEdge), reverseEdge.getMpUsed());
        startingPaths.add(defaultPath);
    ...
    private List<MovePath> generateChildren(MovePath startingPath) {
        ...
        for(int moveType = 0; moveType < moves.size(); moveType++) {
            ...
            MovePath.MoveStepType nextStepType = moves.get(moveType);
            ...
            MovePathFinder.CoordsWithFacing pathDestination = new MovePathFinder.CoordsWithFacing(childPath);
     ...
     */
    private boolean tooMuchTurning(MovePath path, MovePath.MoveStepType stepType) {
        if(path.getLastStep() == null || path.getSecondLastStep() == null) {
            return false;
        }
        
        // more than two turns in a row is no good
        if((stepType == MovePath.MoveStepType.TURN_LEFT || stepType == MovePath.MoveStepType.TURN_RIGHT)
        ...
        if((stepType == MovePath.MoveStepType.TURN_LEFT && path.getLastStep().getType() == MovePath.MoveStepType.TURN_RIGHT) ||
                (stepType == MovePath.MoveStepType.TURN_RIGHT && path.getLastStep().getType() == MovePath.MoveStepType.TURN_LEFT)) {
            return true;
        }
