...
public class MovePathFinder<C> extends AbstractPathFinder<MovePathFinder.CoordsWithFacing, C, MovePath> {
    ...
    public static class NextStepsAdjacencyMap implements AdjacencyMap<MovePath> {
         ...
         * @param stepType
         */
        public NextStepsAdjacencyMap(MovePath.MoveStepType stepType) {
            this.stepType = stepType;
            backwardsStep = stepType == MovePath.MoveStepType.BACKWARDS;
        }
        ...
        public Collection<MovePath> getAdjacent(MovePath mp) {
            ...
            final MovePath.MoveStepType lType = (last == null) ? null : last.getType();
            final Entity entity = mp.getEntity();

            final ArrayList<MovePath> result = new ArrayList<MovePath>();

            if (lType != MovePath.MoveStepType.TURN_LEFT) {
                result.add(mp.clone().addStep(MovePath.MoveStepType.TURN_RIGHT));
            }
            if (lType != MovePath.MoveStepType.TURN_RIGHT) {
                result.add(mp.clone().addStep(MovePath.MoveStepType.TURN_LEFT));
            }
            ...
            if (mp.canShift()) {
                if (backwardsStep) {
                    result.add(mp.clone().addStep(MovePath.MoveStepType.LATERAL_RIGHT_BACKWARDS));
                    result.add(mp.clone().addStep(MovePath.MoveStepType.LATERAL_LEFT_BACKWARDS));
                } else {
                    result.add(mp.clone().addStep(MovePath.MoveStepType.LATERAL_RIGHT));
                    result.add(mp.clone().addStep(MovePath.MoveStepType.LATERAL_LEFT));
                }
            }
            ...
            if (backwardsStep &&
                ...
                result.add(newPath.addStep(MovePath.MoveStepType.BACKWARDS));
            } else if(mp.getGame().getBoard().contains(mp.getFinalCoords().translated(mp.getFinalFacing()))) {
                MovePath newPath = mp.clone();
                PathDecorator.AdjustElevationForForwardMovement(newPath);
                result.add(newPath.addStep(MovePath.MoveStepType.FORWARDS));
            }
